...
async def main_async():
    ...
    USER_ID = "Samiullah Saleem"

    # ===== PART 3: Session Management - Find or Create =====
    # Check for existing sessions for this user
    existing_sessions = await session_service.list_sessions(  # Await here
        app_name=APP_NAME,
        user_id=USER_ID,
    )
    ...
    if existing_sessions and len(existing_sessions.sessions) > 0:
        ...
        new_session = await session_service.create_session(  # Await here
            app_name=APP_NAME,
            user_id=USER_ID,
            state=initial_state,
        )
