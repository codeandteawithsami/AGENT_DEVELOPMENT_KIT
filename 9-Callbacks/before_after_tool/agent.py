...
def before_tool_callback(
    ...
    print(f"[Callback] Origional args '{args}'")
    ...
    print("[Callback] Proceeding with normel tool call")
    return None
...
def after_tool_callback(
    ...
    print(f"[Callback] Origional response: '{tool_response}'")
    
    origional_result = tool_response.get("result", "")
    print(f"[Callback] Extracted result: '{origional_result}'")
    
    # Add a note for any USA capital responses
    if tool_name == "get_capital_city" and "washington" in origional_result.lower():
        ...
        modified_response["result"] = (
            f"{origional_result} (Note: This is the capital of the USA. us)"
        )
    ...
    print("[Callback] Modified response: {modified_response}")
    return None
...
root_agent = LlmAgent(
    ...
    description="An agent that demonstrates tool callbacks by lloking up capital cities",
    instruction="""
